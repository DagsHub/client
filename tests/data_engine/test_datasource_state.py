...
def test_repo_regex(in_str, user, repo, revision, prefix, mock_dagshub_auth):
...
def test_repo_regex_incorrect(in_str, mock_dagshub_auth):
    ds = DatasourceState(repo="user/repo")
    ds.path = in_str
    ds.source_type = DatasourceType.REPOSITORY
...
def test_bucket_regex(in_str, schema, bucket, prefix, mock_dagshub_auth):
    ds = DatasourceState(repo="user/repo")
    ds.path = in_str
...
def test_bucket_regex_incorrect(in_str, mock_dagshub_auth):
    ds = DatasourceState(repo="user/repo")
    ds.path = in_str
    ds.source_type = DatasourceType.REPOSITORY
    with pytest.raises(InvalidPathFormatError):
